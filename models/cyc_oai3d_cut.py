...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            ...
            feat_reshape = feat.view(feat.shape[0], feat.shape[1]*feat.shape[2]*feat.shape[3], feat.shape[4])
            if num_patches > 0:
...
class GAN(BaseModel):
    ...
    def __init__(self, hparams, train_loader, test_loader, checkpoints):
        ...
        netF = PatchSampleF(use_mlp=False, init_type='normal', init_gain=0.02, gpu_ids=[], nc=256)
        ...
        # save model names
        self.netg_names = {'net_gYX': 'net_gYX', 'netF': 'netF'}
    ...
    def generation(self, batch):
        self.ori = batch['img'][0]
        ori = self.upsample(self.ori)  # (B, C, H, W, D)
        #print(ori.shape)
        xy = ori.permute(4, 1, 2, 3, 0)[::1, :, :, :, 0]  # (D, C, H, W)
        #yz = ori.permute(2, 1, 4, 3, 0)[::1, :, :, :, 0]  # (H, C, D, W)

        self.oriX = xy  # 2D
        self.oriY = ori  # 3D

        #self.imgXY = self.net_gXY(self.oriX)['out0']
        self.imgYX3d = self.net_gYX(ori)['out0']  # (B, C, H, W, D)  3D
        self.imgYX = self.imgYX3d.permute(2, 1, 4, 3, 0)[::1, :, :, :, 0]  # (H, C, D, W)
    ...
    def backward_g(self):
        ...
        # ADV(YX)+
        loss_g_gan = self.add_loss_adv(a=self.imgYX, net_d=self.net_dX, truth=True)

        loss_g_l1 = self.add_loss_l1(a=self.imgYX[:,:,::8,:].permute(1, 0, 3, 2), b=self.ori[0,:,:,:,:]) * self.hparams.lamb
        ...
        # CUT NCE_loss
        feat_q = self.net_gYX(self.oriY, encode_only=True)
        feat_k = self.net_gYX(self.imgYX3d, encode_only=True)
...
# USAGE
# CUDA_VISIBLE_DEVICES=0,1,2,3 python train.py --jsn cyc_imorphics --prj cyc_oai3d_cut/0/ --models cyc_oai3d_cut --cropz 16 --cropsize 128 --netG ed03d --env a6k
