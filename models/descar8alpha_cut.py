...
class GAN(BaseModel):
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--alpha", dest='alpha', type=int, help='ending epoch for decaying skip connection, 0 for no decaying', default=0)
        parser.add_argument("--nomask", dest='nomask', action='store_true', default=False, help='do not use masking')
    ...
    def generation(self, batch):
        ...
        outXa = self.net_g(self.oriX, alpha=alpha)['out0']
        if not self.hparams.nomask:
            self.imgXY = nn.Sigmoid()(outXa)  # mask
    ...
    def backward_g(self):
        ...
        if self.hparams.lbNCE > 0:
            feat_q = self.net_g(self.oriX, method='encode')
            ...
            for f_q, f_k, crit in zip(feat_q_pool, feat_k_pool, self.criterionNCE):
                loss = crit(f_q, f_k) * self.hparams.lbNCE
...
# CUDA_VISIBLE_DEVICES=0,1,2,3 python train.py --jsn womac4 --prj 0623/alpha0detach/  --models descar4alpha --netG edalpha --dataset womac4 --split a