...
class GAN(BaseModel):
    ...
    def generation(self, batch):
        if 0:
            mask = batch['img'][0]
            c = (mask == 1) | (mask == 2) | (mask == 3)
            pc = (mask == 4)
            m = (mask == 5) | (mask == 6)
            ori = torch.cat([c, pc, m], 1).float()
        else:
            ori = batch['img'][0]

        ori = self.upsample(ori)
        #print(ori.shape)
        xy = ori.permute(4, 1, 2, 3, 0)[::1, :, :, :, 0]
        yz = ori.permute(2, 1, 3, 4, 0)[::1, :, :, :, 0]
...
# USAGE
# CUDA_VISIBLE_DEVICES=1 python train.py --jsn cyc_imorphics --prj cyc_imorphics/0/
