...
class GAN(BaseModel):
    ...
    def backward_g(self):
        ...
        if self.hparams.lbNCE > 0:
            # (Y, YX)
            feat_yyx = self.net_gYX(torch.cat([self.oriYw, self.oriYo], 1), method='encode')
            ...
            feat_yyx_pool, _ = self.netF(feat_yyx, self.hparams.num_patches, sample_ids)  # use the ids for query target

            total_nce_loss = 0.0
            for f_q, f_k, crit, f_w in zip(feat_yyx_pool, feat_k_pool, self.criterionNCE, self.hparams.fWhich):
            ...
            # (Y, idtY)
            feat_k = self.net_gXY(torch.cat([self.idt_Yw, self.idt_Yo], 1), method='encode')
