...
class GAN(BaseModel):
    ...
    def generation(self, batch):
        self.ori = batch['img'][0] # (B, C, H, W, D)
        ...
        self.imgYX = self.net_g(ori)['out0']  # (B, C, H, W, D)
        self.imgYX = self.imgYX.permute(2, 1, 4, 3, 0)[::1, :, :, :, 0]  # (H, C, D, W)


        #if self.hparams.lamb > 0:
        #    self.imgXYX = self.net_g(self.imgXY)['out0']
        #    self.imgYXY = self.net_gXY(self.imgYX)['out0']

        #if self.hparams.lambI > 0:
        #    self.idt_X = self.net_g(self.oriX)['out0']
    ...
    def backward_g(self):
        ...
        # ADV(YX)+
        loss_g_gan = self.add_loss_adv(a=self.imgYX, net_d=self.net_d, truth=True)
    ...
    def backward_d(self):
        ...
        # ADV(YX)-
        loss_d += self.add_loss_adv(a=self.imgYX, net_d=self.net_d, truth=False)
