...
class GAN(BaseModel):
    ...
    def backward_g(self):
        ...
        if self.hparams.lbS > 0:
            ...
            crit = SupConLoss()
            labels = torch.tensor([1, 1, 0, 0, 0, 0]).to(self.device)
            feat_p1 = self.net_g(self.oriX[:23], method='encode')[-1]
            feat_p2 = self.net_g(self.oriX[23:], method='encode')[-1]
            feat_np1 = self.net_g(self.oriY[:23], method='encode')[-1]
            feat_np2 = self.net_g(self.oriY[23:], method='encode')[-1]
            feat_fp1 = self.net_g(self.imgXY[:23], method='encode')[-1]
            feat_fp2 = self.net_g(self.imgXY[23:], method='encode')[-1]
            feats = [feat_p1, feat_p2, feat_np1, feat_np2, feat_fp1, feat_fp2]
            feats = [featDown(f) for f in feats]
            feats = [f.flatten().unsqueeze(0).unsqueeze(0) for f in feats]
            feats = torch.cat(feats, 0) #torch.Size([6, 1, 5888])
        ...
        if self.hparams.lbT > 0:
            total_tri_loss = 0.0
            # anchor=p1', positive=np2, negative=p2
        ...
        return {'sum': loss_g, 'l1': loss_l1, 'ga': loss_ga, 'gvgg': loss_gvgg, 'loss_nce': loss_nce,
                'loss_tri': loss_tri, 'loss_sup': loss_sup}
