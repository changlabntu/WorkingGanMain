...
class GAN(BaseModel):
    ...
    def __init__(self, hparams, train_loader, test_loader, checkpoints):
        ...
        self.netG, _ = self.set_networks()
        self.netF = networks.define_F(opt.input_nc, opt.netF, opt.normG, not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids, opt)

        self.netGYX = copy.deepcopy(self.netG)
        ...
        if self.isTrain:
            ...
            self.netDX = copy.deepcopy(self.netD)
        ...
        self.netg_names = {'netG': 'netG', 'netF': 'netF', 'netGYX': 'netGYX'}
        self.netd_names = {'netD': 'netD', 'netDX': 'netDX'}
    ...
    def data_dependent_initialize(self):
        ...
        feature_shapes = [(16, 32, 256, 256), (16, 64, 128, 128), (16, 128, 64, 64), (16, 256, 64, 64)]
    ...
    def generation(self, batch):
        ...
        # CYC
        self.fake_A = self.netGYX(self.real_B)
        if self.hparams.lamb > 0:
            self.imgXYX = self.netGYX(self.fake_B)
            self.imgYXY = self.netG(self.fake_A)

        if self.hparams.lambI > 0:
            self.idt_X = self.netGYX(self.real_A)
            self.idt_Y = self.netG(self.real_B)
    ...
    def backward_d(self):
        ...
        # ADV(YX)-
        loss_d_cyc = 0
        pred_fake = self.netDX(self.fake_A.detach())
        loss_d_cyc += self.criterionGAN(pred_fake, False).mean()
        # ADV(X)+
        pred_real = self.netDX(self.real_A)
        loss_d_cyc += self.criterionGAN(pred_real, True)

        # combine loss and calculate gradients
        self.loss_D = (self.loss_D_fake + self.loss_D_real) * 0.5 + loss_d_cyc * 0.5

        return {'sum': self.loss_D, 'd_real': self.loss_D_real, 'd_fake': self.loss_D_fake, 'd_cyc': loss_d_cyc}
    ...
    def backward_g(self):
        ...
        loss_g_cyc = 0
        if self.hparams.lamb > 0:
            loss_g_cyc += self.add_loss_l1(a=self.imgXYX, b=self.real_A) * self.hparams.lamb
            # Cyclic(YXY, Y)
            loss_g_cyc += self.add_loss_l1(a=self.imgYXY, b=self.real_B) * self.hparams.lamb

        # Identity(idt_X, X)
        if self.hparams.lambI > 0:
            loss_g_cyc += self.add_loss_l1(a=self.idt_X, b=self.real_A) * self.hparams.lambI
            # Identity(idt_Y, Y)
            loss_g_cyc += self.add_loss_l1(a=self.idt_Y, b=self.real_B) * self.hparams.lambI

        self.loss_G = self.loss_G_GAN + loss_NCE_both + loss_g_cyc
        #print((self.loss_G_GAN, loss_NCE_both))

        return {'sum': self.loss_G, 'g_gan': self.loss_G_GAN, 'nce': loss_NCE_both, 'g_cyc': loss_g_cyc}
