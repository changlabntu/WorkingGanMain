...
def conv2d_bn_block(in_channels, out_channels, kernel=3, stride=1, momentum=0.01, activation=ACTIVATION):
    return nn.Sequential(
...
class Generator(nn.Module):
    def __init__(self, n_channels=1, out_channels=1, nf=32, batch_norm=True, activation=ACTIVATION, final='tanh', mc=False):
        ...
        self.down0 = nn.Sequential(
            conv2_block(n_channels, nf, stride=2, activation=act),
            conv2_block(nf, nf, activation=act)
        )
        self.down1 = nn.Sequential(
            #max2_pool,
            conv2_block(nf, 2 * nf, activation=act),
            conv2_block(2 * nf, 2 * nf, stride=2, activation=act),
        )
...
if __name__ == '__main__':
    ...
    f = g(torch.rand(1, 3, 256, 256, 128), method='encode')
    for i in f:
        print(i.shape)
