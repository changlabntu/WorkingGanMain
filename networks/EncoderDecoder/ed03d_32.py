...
def conv3d_bn_block(in_channels, out_channels, kernel=3, stride=1, momentum=0.01, activation=ACTIVATION):
...
class Generator(nn.Module):
    def __init__(self, n_channels=1, out_channels=1, nf=32, batch_norm=True, activation=ACTIVATION, final='tanh', mc=False):
        ...
        self.down0 = nn.Sequential(
            conv_block(n_channels, nf, stride=2, activation=act),
            conv_block(nf, nf, stride=2, activation=act)
        )
        ...
        self.conv7_k = nn.Sequential(
            deconv3d_bn_block(nf, nf, activation=act),
            deconv3d_bn_block(nf, out_channels, activation=final_layer),
        )
...
if __name__ == '__main__':
    ...
    out = g(torch.rand(1, 3, 128, 128, 23*8))
    print(out['out0'].shape)
    f = g(torch.rand(1, 3, 128, 128, 23*8), method='encode')
    for i in f:
        print(i.shape)