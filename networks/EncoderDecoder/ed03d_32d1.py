...
class Generator(nn.Module):
    def __init__(self, n_channels=1, out_channels=1, nf=32, batch_norm=True, activation=ACTIVATION, final='tanh', mc=False):
        ...
        self.down0 = nn.Sequential(
            conv_block(n_channels, nf, stride=(2, 2, 1), activation=act),
            conv_block(nf, nf, stride=1, activation=act)
        )
        ...
        self.conv7_k = nn.Sequential(
            deconv3d_bn_block(nf, out_channels, use_upsample=(2, 2, 1), activation=final_layer),
        )
...
if __name__ == '__main__':
    ...
    out = g(torch.rand(1, 3, 128, 128, 22*8))
    print(out['out0'].shape)
    f = g(torch.rand(1, 3, 128, 128, 128), method='encode')
    for i in f:
        print(i.shape)