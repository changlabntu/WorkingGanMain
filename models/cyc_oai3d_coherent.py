...
class GAN(BaseModel):
    ...
    def backward_g(self):
        ...
        loss_g_gan = 0
        loss_g_gan += self.add_loss_adv(a=self.imgYX.permute(4, 1, 2, 3, 0)[:, :, :, :, 0],  # (Z, C, X, Y)
    ...
    def backward_d(self):
        ...
        loss_d_gan = 0
        loss_d_gan += self.add_loss_adv(a=self.imgYX.permute(4, 1, 2, 3, 0)[:, :, :, :, 0],  # (Z, C, X, Y)
...
# USAGE
# CUDA_VISIBLE_DEVICES=0,1 python train.py --jsn cyc_imorphics --prj cyc_oai3d_coherant/23d/ --models cyc_oai3d_coherent --cropz 16 --cropsize 128 --netG ed023d --trd 800