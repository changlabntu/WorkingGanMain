...
class GAN(BaseModel):
    ...
    def backward_g(self):
        ...
        if self.hparams.lbNCE > 0:
            ...
            # (X, idtX)
            feat_q = self.net_gYX(torch.cat([self.oriXw, self.oriXo], 1), method='encode')
            feat_k = self.net_gYX(torch.cat([self.idt_Xw, self.idt_Xo], 1), method='encode')

            feat_k_pool, sample_ids = self.netFB(feat_k, self.hparams.num_patches,
                                                None)  # get source patches by random id
            feat_q_pool, _ = self.netFB(feat_q, self.hparams.num_patches, sample_ids)  # use the ids for query target
