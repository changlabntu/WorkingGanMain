...
class GAN(BaseModel):
    ...
    def generation(self, batch):
        ...
        self.t2dLR = self.net_gLR(self.tLR)['out0']

        self.t2dLRHR = torch.nn.Upsample(scale_factor=2.0, mode='bilinear')(self.t2dLR)

        self.t2d = self.net_g(torch.cat([self.t, self.t2dLRHR], 1))['out0']
...
# CUDA_VISIBLE_DEVICES=0,1 python train.py --jsn t2d --prj cascade0d2t/  --models t2d0cascade --netG dsmc --netD bpatch_16 --split a --dataset t2d --direction d_tres -b 16 --split all --dataset_mode PairedSlices